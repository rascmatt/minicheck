module Lib.Parser.Options (Options, oArgs, oTsFile, oHelp, oIllegal, parse) where
...
parse args = Options ar tsFile hFlag illegal
    where
        parsed  = [ (a, arg a, option "ts" a, flag "help" a) | a <- args ]
        -- Collect everything we can't parse at all
        illegal = [ a | (a, Nothing, Nothing, Nothing) <- parsed ]
        -- If we can't parse it to a flag or an option, its a normal argument
        ar      = [ x | (_, Just x, Nothing, Nothing) <- parsed ]
        ts      = [ tt | (_, _, Just tt, _) <- parsed ]
        -- Use the last occurrence of the '--ts' flag
        tsFile  = if null ts then [] else last ts
        -- Use the last occurrence of the '--help' flag
        help    = [ x | (_, Nothing, Nothing, Just x) <- parsed ]
